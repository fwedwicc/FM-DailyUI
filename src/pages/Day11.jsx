    ...
    >
      <section className="bg-neutral-800 min-h-screen flex items-center justify-center p-12 md:px-12 px-8 py-8 overflow-x-hidden">
        <div className='flex items-center justify-center md:flex-nowrap flex-wrap gap-12 lg:gap-20'>
          {/* Phone Mockup */}
          <div className='relative border-[0.5rem] border-neutral-900 h-[40rem] w-[21.5rem] rounded-[2rem] ring-2 ring-green-500'>
            {/* Sound Buttons */}
            <div className='absolute top-20 -left-3 flex flex-col gap-3'>
              {[0, 1, 2].map((index) => (
                <div className={`rounded-full ${index === 0 ? 'h-7' : 'h-10'} w-2 border-l-2 border-green-500 bg-neutral-900`} key={index}></div>
              ))}
            </div>
            {/* Power Button */}
            <div className='absolute top-[9.2rem] -right-3 rounded-full h-16 w-2 border-r-2 border-green-500 bg-neutral-900'></div>
            {/* Camera */}
            ...
            <div className='text-xs'>
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati minus sequi fugit corrupti magni reprehenderit cumque! Et veniam odio ratione dolorum velit libero, magni aut rerum recusandae cupiditate, dolorem excepturi.
            </div>
            {/* Phone Navigation */}
            <div className='absolute rounded-full h-1.5 w-24 bottom-2 left-1/2 transform -translate-x-1/2 bg-neutral-900'></div>
          </div>
          <div className='border h-[40rem] w-[21.5rem] rounded-[2rem]'>
